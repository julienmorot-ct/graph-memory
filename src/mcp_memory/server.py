...
from .auth.context import check_memory_access, check_write_permission
...
async def memory_create(
    memory_id: str, name: str, ontology: str, description: Optional[str] = None
) -> dict:
    """
    ...
    try:
        ...
        print(
            f"📝 [Memory] Ontologie '{ontology}' stockée: {ontology_s3_result['uri']}",
            file=sys.stderr,
        )
...
async def memory_delete(memory_id: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            print(
                f"❌ [Qdrant] Erreur suppression collection pour {memory_id}: {e}", file=sys.stderr
            )
            raise RuntimeError(
                f"Impossible de supprimer la collection Qdrant (couplage strict): {e}"
            )
    ...
    try:
        ...
        async def _extraction_progress(event: str, data: dict):
            if event == "extraction_start":
                ...
                await _log(
                    f"🔍 Chunk {chunk}/{total} terminé: +{e_new}E +{r_new}R (cumul: {e_cum}E {r_cum}R)"
                )
        ...
        await _log(
            f"✅ Extraction terminée: {len(extraction.entities)} entités, {len(extraction.relations)} relations"
        )
            ...
            if chunks:
                ...
                for batch_start in range(0, total_chunks, EMBED_BATCH_SIZE):
                    ...
                    try:
                        ...
                        await _log(
                            f"✅ Batch {batch_num}/{total_batches} OK ({len(all_embeddings)}/{total_chunks})"
                        )
                        sys.stderr.flush()
                    except Exception as embed_err:
                        print(
                            f"❌ [Ingest] Erreur embedding batch {batch_num}: {embed_err}",
                            file=sys.stderr,
                        )
                        sys.stderr.flush()
                        raise
...
@mcp.tool()
async def memory_search(memory_id: str, query: str, limit: int = 10) -> dict:
    """
...
@mcp.tool()
async def question_answer(memory_id: str, question: str, limit: int = 10) -> dict:
    """
    ...
    try:
        ...
        print(
            f"🔎 [Q&A] Recherche graphe: memory={memory_id}, question='{question}', limit={limit}",
            file=sys.stderr,
        )
        ...
        if entities:
            ...
            print(
                f"📊 [Q&A] Graphe: {len(entities)} entités trouvées → {entity_summary}",
                file=sys.stderr,
            )
        else:
        ...
        try:
            ...
            for i, cr in enumerate(chunk_results):
                ...
                print(
                    f'   📎 [{i + 1}] score={cr.score:.4f} ✅ | {section} | "{preview}..."',
                    file=sys.stderr,
                )
            ...
            if filtered_out > 0:
                ...
                for i, cr in enumerate(filtered_chunks[:5]):  # Max 5 pour ne pas surcharger
                    ...
                    print(
                        f'   📎 [F{i + 1}] score={cr.score:.4f} ❌ | {section} | "{preview}..."',
                        file=sys.stderr,
                    )
...
@mcp.tool()
async def memory_query(memory_id: str, query: str, limit: int = 10) -> dict:
    """
    ...
    try:
        ...
        # 1. Rechercher les entités pertinentes dans le graphe
        print(
            f"🔎 [Query] Recherche graphe: memory={memory_id}, query='{query}', limit={limit}",
            file=sys.stderr,
        )
        entities = await get_graph().search_entities(memory_id, search_query=query, limit=limit)
        ...
        if entities:
            entity_summary = ", ".join(f"{e['name']} ({e.get('type', '?')})" for e in entities)
            print(
                f"📊 [Query] Graphe: {len(entities)} entités trouvées → {entity_summary}",
                file=sys.stderr,
            )
        else:
...
@mcp.tool()
async def memory_get_context(memory_id: str, entity_name: str, depth: int = 1) -> dict:
    """
...
@mcp.tool()
async def document_get(memory_id: str, document_id: str, include_content: bool = False) -> dict:
    """
...
async def document_delete(memory_id: str, document_id: str) -> dict:
    ...
    try:
        ...
        try:
            qdrant_chunks_deleted = await get_vector_store().delete_document_chunks(
                memory_id, document_id
            )
        except Exception as e:
            print(
                f"❌ [Qdrant] Erreur suppression chunks pour doc {document_id}: {e}",
                file=sys.stderr,
            )
    ...
    try:
        ...
        report = {
            ...
            "summary": (
                ...
                + (
                    f", ❌ {check_result['missing']} manquants"
                    if check_result["missing"] > 0
                    else ""
                )
                + (
                    f", ⚠️ {len(orphans)} orphelins S3 ({_human_size(orphan_total_size)})"
                    if orphans
                    else ""
                )
            ),
...
async def backup_create(
    memory_id: str, description: Optional[str] = None, ctx: Optional[Context] = None
) -> dict:
    """
...
@mcp.tool()
async def backup_restore(backup_id: str, ctx: Optional[Context] = None) -> dict:
    """
...
async def backup_download(
    backup_id: str, include_documents: bool = False, ctx: Optional[Context] = None
) -> dict:
    """
...
@mcp.tool()
async def backup_restore_archive(archive_base64: str, ctx: Optional[Context] = None) -> dict:
    """
    ...
    try:
        ...
        result = await get_backup().restore_from_archive(
            archive_bytes=archive_bytes, progress_callback=_progress
        )
        return result
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
def main():
    ...
    print(
        "  - admin_create_token, admin_list_tokens, admin_revoke_token, admin_update_token",
        file=sys.stderr,
    )
    print("  - storage_check, storage_cleanup, system_health, system_about", file=sys.stderr)
    print(
        "  - backup_create, backup_list, backup_restore, backup_download, backup_delete",
        file=sys.stderr,
    )
    print("=" * 70, file=sys.stderr)
