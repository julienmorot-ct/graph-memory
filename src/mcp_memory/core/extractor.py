...
class ExtractorService:
    ...
    async def extract_from_text(
        ...
        try:
            ...
            if response.choices:
                print(
                    f"🔍 [Extractor] DEBUG message: {response.choices[0].message}", file=sys.stderr
                )
                print(
                    f"🔍 [Extractor] DEBUG finish_reason: {response.choices[0].finish_reason}",
                    file=sys.stderr,
                )
            ...
            if content is None:
                print(
                    f"⚠️ [Extractor] Réponse LLM vide - message complet: {response.choices[0].message}",
                    file=sys.stderr,
                )
            ...
            print(
                f"✅ [Extractor] Extrait: {len(result.entities)} entités, {len(result.relations)} relations",
                file=sys.stderr,
            )
        ...
        try:
            ...
            for r in data.get("relations", []):
                rel_type = self._parse_relation_type(
                    r.get("type", "RELATED_TO"), known_types=known_relation_types
                )
                relations.append(
                    ExtractedRelation(
    ...
    def _normalize_entity_type(type_str: str, known_types: Optional[set] = None) -> str:
        ...
        print(
            f"⚠️ [Normalize] Type LLM rejeté: '{type_str}' → Other (known: {len(known_types)} types)",
            file=sys.stderr,
        )
        return "Other"
    ...
    async def extract_with_ontology(
        ...
        try:
            print(
                f"🔍 [Extractor] Extraction avec ontologie '{ontology.name}' ({len(text)} chars)...",
                file=sys.stderr,
            )
            ...
            print(
                f"🔗 [Extractor] Types ontologie '{ontology.name}': {len(ontology_entity_types)} entités, {len(ontology_relation_types)} relations",
                file=sys.stderr,
            )
            ...
            print(
                f"✅ [Extractor] Extrait ({ontology.name}): {len(result.entities)} entités, {len(result.relations)} relations",
                file=sys.stderr,
            )
    ...
    async def extract_with_ontology_chunked(
        ...
        if len(text) <= chunk_size:
            print(
                f"📄 [Extractor] Document court ({len(text)} chars ≤ {chunk_size}) → extraction simple",
                file=sys.stderr,
            )
        ...
        print(
            f"📐 [Extractor] Document long ({len(text)} chars) → {len(chunks)} chunks d'extraction",
            file=sys.stderr,
        )
        ...
        if not ontology:
            available = [o["name"] for o in ontology_manager.list_ontologies()]
            raise ValueError(
                f"Ontologie '{ontology_name}' introuvable. Ontologies disponibles: {available}."
            )
        ...
        for i, chunk_text in enumerate(chunks):
            ...
            print(
                f"🔄 [Extractor] Chunk {chunk_num}/{len(chunks)} "
            ...
            try:
                ...
                print(
                    f"✅ [Extractor] Chunk {chunk_num}: +{len(result.entities)} entités, "
                ...
                print(
                    f"⏰ [Extractor] Timeout chunk {chunk_num}/{len(chunks)} — on continue",
                    file=sys.stderr,
                )
                ...
                print(
                    f"❌ [Extractor] Erreur API chunk {chunk_num}/{len(chunks)}: {e}",
                    file=sys.stderr,
                )
                raise
        ...
        merged = self._merge_extraction_results(
            all_entities, all_relations, all_summaries, all_key_topics
        )

        print(
            f"🏁 [Extractor] Extraction chunked terminée: "
            f"{len(merged.entities)} entités, {len(merged.relations)} relations "
            f"(depuis {len(chunks)} chunks)",
            file=sys.stderr,
        )
        ...
        for e in all_entities:
            ...
            if key not in entity_map:
                ...
                if e.description and (
                    not existing.description or len(e.description) > len(existing.description)
                ):
        ...
        for r in all_relations:
            ...
            if key not in relation_map:
                ...
                if r.description and (
                    not existing.description or len(r.description) > len(existing.description)
                ):
    ...
    async def test_connection(self) -> dict:
        ...
        try:
            ...
            return {"status": "ok", "model": self._model, "message": "Connexion LLMaaS réussie"}

        except APIError as e:
            return {"status": "error", "model": self._model, "message": f"Erreur LLMaaS: {str(e)}"}
