...
class BackupService:
    ...
    def _validate_backup_id(backup_id: str) -> tuple:
        ...
        if len(parts) != 2:
            raise ValueError(
                f"backup_id invalide: '{backup_id}'. Format attendu: 'MEMORY_ID/TIMESTAMP'"
            )
    ...
    async def create_backup(
        self, memory_id: str, description: Optional[str] = None, progress_callback=None
    ) -> Dict[str, Any]:
        """
        ...
        await _log(
            f"✅ Graphe: {graph_stats['entities']} entités, "
        ...
        await _log(
            f"✅ Documents: {len(document_keys)} références ({self._human_size(total_doc_size)})"
        )
            ...
            if obj["key"].endswith("/manifest.json"):
                try:
                    ...
                    response = self._storage._client.get_object(
                        Bucket=self._storage._bucket, Key=obj["key"]
                    )
                    ...
                    print(f"⚠️ [Backup] Erreur lecture manifest {obj['key']}: {e}", file=sys.stderr)
    ...
    async def restore_backup(self, backup_id: str, progress_callback=None) -> Dict[str, Any]:
        """
        ...
        await _log(
            f"✅ Manifest OK: {manifest.get('memory_name', '?')} "
        ...
        if expected_hash and actual_hash != expected_hash:
            raise ValueError("Checksum vecteurs invalide ! Le backup est peut-être corrompu.")
    ...
    async def download_backup(
        self, backup_id: str, include_documents: bool = False, progress_callback=None
    ) -> bytes:
        """
        ...
        with tarfile.open(fileobj=buf, mode="w:gz") as tar:
            ...
            for filename in json_files:
                ...
                try:
                    response = self._storage._client.get_object(
                        Bucket=self._storage._bucket, Key=key
                    )
                    content = response["Body"].read()
            ...
            if include_documents:
                ...
                doc_keys_text = await self._download_text(f"{backup_prefix}/document_keys.json")
                ...
                for i, doc in enumerate(doc_keys):
                    ...
                    try:
                        ...
                        info = tarfile.TarInfo(name=f"{archive_dir}/documents/{filename_orig}")
                        info.size = len(content)
                        tar.addfile(info, io.BytesIO(content))
    ...
    async def restore_from_archive(
        self, archive_bytes: bytes, progress_callback=None
    ) -> Dict[str, Any]:
        """
        ...
        await _log(
            f"✅ Manifest OK: mémoire '{memory_id}' "
            f"({manifest['stats']['entities']} entités, "
            f"{manifest['stats']['qdrant_vectors']} vecteurs)"
        )
        ...
        if existing:
            tar.close()
            raise ValueError(
                f"La mémoire '{memory_id}' existe déjà. Supprimez-la d'abord avec memory_delete."
            )
        ...
        for doc_member in doc_members:
            ...
            if ".." in doc_filename or doc_filename.startswith("/"):
                print(
                    f"🔒 [RestoreArchive] Nom de fichier rejeté (path traversal): '{doc_filename}'",
                    file=sys.stderr,
                )
                docs_skipped += 1
                continue
            ...
            if safe_filename != doc_filename:
                print(
                    f"🔒 [RestoreArchive] Nom normalisé: '{doc_filename}' → '{safe_filename}'",
                    file=sys.stderr,
                )
            ...
            if s3_key:
                ...
                try:
                    ...
                    self._storage._client.put_object(
                        ...
                        Metadata={
                            "memory_id": memory_id,
                            "original_filename": self._storage._sanitize_metadata_value(
                                doc_filename
                            ),
                ...
                try:
                    await self._storage.upload_document(
                        memory_id=memory_id, filename=doc_filename, content=doc_content
                    )
                    docs_uploaded += 1
    ...
    async def delete_backup(self, backup_id: str) -> Dict[str, Any]:
        ...
        print(
            f"🗑️ [Backup] Supprimé: {backup_id} ({result['deleted_count']} fichiers)",
            file=sys.stderr,
        )
        ...
        for backup in to_delete:
            ...
            if bid:
                try:
                    ...
                    print(f"⚠️ [Retention] Erreur suppression {bid}: {e}", file=sys.stderr)
    ...
    async def _download_text(self, key: str) -> str:
        ...
        try:
            response = self._storage._client.get_object(Bucket=self._storage._bucket, Key=key)
