...
class Ontology:
    ...
    def build_prompt(self, document_text: str, cumulative_context: str = "") -> str:
        ...
        if priority_entities or self.extraction_rules.priority_entities:
            priority_types = priority_entities or [
                et for et in self.entity_types if et.name in self.extraction_rules.priority_entities
            ]
            ...
            if filename.endswith(".yaml") or filename.endswith(".yml"):
                ...
                try:
                    ...
                    if ontology:
                        ...
                        print(
                            f"✅ [Ontology] Chargée: {ontology.name} (v{ontology.version})",
                            file=sys.stderr,
                        )
        ...
        return [
            {
                "name": ont.name,
                "version": ont.version,
                "description": ont.description.strip()[:100] + "..."
                if len(ont.description) > 100
                else ont.description.strip(),
