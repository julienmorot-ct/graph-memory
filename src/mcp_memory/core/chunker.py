...
NUMBERED_SECTION_PATTERN = re.compile(r"^(\d+(?:\.\d+)+)\s*[:\.\s–—-]", re.MULTILINE)
...
UPPERCASE_TITLE_PATTERN = re.compile(
    r"^([A-ZÀÂÄÉÈÊËÏÎÔÙÛÜŸÇ][A-ZÀÂÄÉÈÊËÏÎÔÙÛÜŸÇ\s,\'-]{15,})$", re.MULTILINE
)

# Séparateurs de phrases (pour le split au niveau phrase)
SENTENCE_ENDINGS = re.compile(r"(?<=[.!?])\s+(?=[A-ZÀÂÄÉÈÊËÏÎÔÙÛÜŸ])")
...
class SemanticChunker:
    ...
    def chunk_document(self, text: str, filename: str) -> List[Chunk]:
        ...
        print(
            f"📐 [Chunker] PASSE 1/3 — {len(sections)} sections détectées dans '{filename}' ({total_chars} chars)",
            file=sys.stderr,
        )
        sys.stderr.flush()
        for i, s in enumerate(sections):
            ...
            print(
                f"   📄 [{i + 1}/{len(sections)}] {s.title[:70]}{art} — {len(s.content)} chars, level={s.level}",
                file=sys.stderr,
            )
        sys.stderr.flush()
        ...
        print(
            f"📐 [Chunker] PASSE 2/3 — {total_sentences} phrases dans {len(sentence_groups)} groupes",
            file=sys.stderr,
        )
        sys.stderr.flush()
        ...
        print(
            f"📐 [Chunker] PASSE 3/3 — Fusion en chunks (cible: {self._chunk_size} tokens, overlap: {self._chunk_overlap})...",
            file=sys.stderr,
        )
        sys.stderr.flush()
        ...
        print(
            f"✅ [Chunker] '{filename}' → {len(chunks)} chunks "
            f"(cible: {self._chunk_size} tokens, overlap: {self._chunk_overlap})",
            file=sys.stderr,
        )
    ...
    def _detect_articles(self, text: str) -> List[TextSection]:
        ...
        if matches[0].start() > 0:
            ...
            if preamble:
                sections.append(
                    TextSection(title="Préambule", content=preamble, level=0, start_pos=0)
                )
    ...
    def _detect_markdown_headers(self, text: str) -> List[TextSection]:
        ...
        if matches[0].start() > 0:
            ...
            if preamble:
                sections.append(
                    TextSection(title="Introduction", content=preamble, level=0, start_pos=0)
                )
    ...
    def _detect_numbered_sections(self, text: str) -> List[TextSection]:
        ...
        for i, match in enumerate(matches):
            ...
            sections.append(
                TextSection(
                    title=title, content=content, level=level, article_number=num, start_pos=start
                )
            )
    ...
    def _detect_uppercase_titles(self, text: str) -> List[TextSection]:
        ...
        for i, match in enumerate(matches):
            ...
            sections.append(TextSection(title=title, content=content, level=0, start_pos=start))
    ...
    def _merge_into_chunks(self, groups: List[SentenceGroup]) -> List[Tuple[SentenceGroup, str]]:
        """
    ...
    def _split_group_with_overlap(self, group: SentenceGroup) -> List[Tuple[SentenceGroup, str]]:
        """
